...
# Warn about forbidden projects when bad IDs are entered.
func ProjectIDEntered(Toggled) -> void:
	if Toggled:
		return;

	if Constants.BLACKLISTED_IDS.keys().has(%ProjectID.text):
		Dialog.ShowMessagePopup("Alert", "This model should not be printed!\n" + Constants.BLACKLISTED_IDS[%ProjectID.text])
...
func Open() -> void:
	...
	Animate.SlideTo(%SlideMenu, Vector2(0, 0), 0.5)
	%Main.LoadProjects()

# Close project menu animation.
func Close() -> void:
	# Slide back to the main menu.
	Animate.SlideTo(%SlideMenu, Vector2(1280, 0), 0.5)
	%Delete.text = "Delete"
	ConfirmDelete = false
	IsNewProject = false
	%Main.LoadProjects()
